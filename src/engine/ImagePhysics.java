...
public class ImagePhysics {
    ...
    public static int getARGBInt(javafx.scene.paint.Color c){
        return (int)(c.getOpacity() * 255) << 24 | (int)(c.getRed()*255) << 16  | (int)(c.getGreen()*255) << 8 | (int)(c.getBlue()*255);
    }
    ...
    public static Vec2f getCollisionPoint(ImageObject o1, ImageObject o2){
        //no collision.
        ...
        for(int x = minX; x < maxX; x++){
            for(int y = minY;  y < maxY; y++){
                if(getARGBAlpha(o1.getGlobalRGB(x, y)) != 0 && getARGBAlpha(o2.getGlobalRGB(x, y)) != 0)
                {
                    ...
                    o1.pixelCollision(o2,x,y);
                    o2.pixelCollision(o1,x,y);
        ...
        //adds 0.5 to put the collision point in center of the pixel
        collisionPoint.add(0.5f,0.5f);

        return collisionPoint;
    }

    public static Vec2f getRelativeCollisionPoint(ImageObject o1, ImageObject o2){
        //no collision.
        if(o1.getMinX() > o2.getMaxX() || o2.getMinX() > o1.getMaxX() || o1.getMinY() > o2.getMaxY() || o2.getMinY() > o1.getMaxY())
            return null;
        int dx = o2.getPosition().x-o1.getPosition().x >0?(int)(o2.getPosition().x-o1.getPosition().x): (int)(o2.getPosition().x-o1.getPosition().x)-1;
        int dy = o2.getPosition().y-o1.getPosition().y >0?(int)(o2.getPosition().y-o1.getPosition().y): (int)(o2.getPosition().y-o1.getPosition().y)-1;

        int minX = dx < 0? dx: 0;
        int maxX = max((int)o1.getImage().getWidth()+dx, (int)o2.getImage().getWidth());
        int minY = dy < 0? dy: 0;
        int maxY = max((int)o1.getImage().getHeight()+dy, (int)o2.getImage().getHeight());

        int collisionCount = 0;
        Vec2f collisionPoint = new Vec2f();
        for(int x = minX; x < maxX; x++)
            for(int y = minY; y < maxY; y++)
                if(getARGBAlpha(o1.getImage().getPixelReader().getArgb(x,y)) != 0 && getARGBAlpha(o2.getImage().getPixelReader().getArgb(x - dx, y - dy)) != 0) {
                    collisionPoint.add(x, y);
                    collisionCount++;
                }
        ...
        //adds the round for the collision
        collisionPoint.add((o2.getPosition().x-o1.getPosition().x)-dx,(o2.getPosition().y-o1.getPosition().y)-dy);
        //adds 0.5 to put the collision point in center of the pixel
        collisionPoint.add(0.5f,0.5f);
        return collisionPoint;
    }
    ...
    public static void handleCollision(ImageObject o1, ImageObject o2, float deltaTime){
        ...
        // SOME EXPERIMENT
        Vec2f collisionPoint = getCollisionPoint(o1, o2);
        if(collisionPoint == null)
            return;

        CollisionDirective o1cd = o1.preCollideWith(o2,collisionPoint,deltaTime);
        CollisionDirective o2cd = o2.preCollideWith(o1,collisionPoint,deltaTime);

        //which one shall we move?
        boolean moveo1 = o1cd.moveThis && o2cd.moveObj && !o1.isStuck();
        boolean moveo2 = o2cd.moveThis && o1cd.moveObj && !o2.isStuck();
        ...
        //the border from collisionPoint to border on object // we have landed on a rounded line.
        Vec2f o1cptob = searchBorder(collisionPoint, o1.getVelocity(), o1).sub(collisionPoint);
        Vec2f o2cptob = searchBorder(collisionPoint, o2.getVelocity(), o2).sub(collisionPoint);

        //speed of objects
        float o1s = o1.getVelocity().len();
        float o2s = o2.getVelocity().len();
/*
        if(o1s > o2s)
        {
            float kvot = /;
 ...
 /*       if(Math.abs(o1cb.x- Math.round(o1cb.x)) < Math.abs(o1cb.y- Math.round(o1cb.y)) && Math.abs(o2cb.x- Math.round(o2cb.x)) < Math.abs(o2cb.y- Math.round(o2cb.y))){
            System.out.println("hej");
        }
*/

        //deltapositions
        Vec2f o1dp = mult(deltaTime, o1.getVelocity());
        Vec2f o2dp = mult(deltaTime, o2.getVelocity());

        //center to collisionpoints
        Vec2f o1ctocp = sub(collisionPoint,o1.getCenter());
        Vec2f o2ctocp = sub(collisionPoint,o2.getCenter());

        //acceptable movements //here friction comes in
        Vec2f o1am = sub(o1dp, proj(o1dp,o1ctocp));
        Vec2f o2am = sub(o2dp, proj(o2dp,o2ctocp));


        //Velocity projection on center to collisionPoint.
        Vec2f o1vonctocp = proj(o1.getVelocity(),o1ctocp);
        Vec2f o2vonctocp = proj(o1.getVelocity(),o2ctocp);
        ///////Normal force (the the with which the two objects collide)
        //Assumes that previous speed were 0 to calculate acceleration.
        float collisionForce = o1vonctocp.len()/deltaTime*o1.getMass() + o2vonctocp.len()/deltaTime*o2.getMass();
        float frictionForce = collisionForce*o1cd.friction * o2cd.friction;

        //movement in int position
        int o1dx = round(o1.getPosition().x) - round(o1.getPosition().x - o1dp.x);
        int o1dy = round(o1.getPosition().y) - round(o1.getPosition().y - o1dp.y);
        int o2dx = round(o2.getPosition().x) - round(o2.getPosition().x - o2dp.x);
        int o2dy = round(o2.getPosition().y) - round(o2.getPosition().y - o2dp.y);
...
/*
        if(!o1.isStuck()){
            o1.getPosition().sub(o1cptob);
            o1.getPosition().add(o1am);
        }
        if(!o2.isStuck()){
            o2.getPosition().sub(o2cptob);
            o2.getPosition().add(o2am);
        }
*/

        if(!o1.isStuck()){
            o1.getPosition().sub(o1dp);
            //o1.getPosition().add(o1am);
        }
        if(!o2.isStuck()) {
            o2.getPosition().sub(o2dp);
            //o2.getPosition().add(o2am);
        }
     ...
     */
    public static WritableImage trimedImage(Image img) {
        ...
        for (i = 0; i < width; i++) {
            for (j = 0; j < height; j++) {
                if (getARGBAlpha(pr.getArgb(i,j)) != 0) {
                    break leftLoop;
                }
            }
        }
        ...
        y1 = j+1;
        return new WritableImage(pr, x0, y0, x1-x0, y1-y0);
    }
    ...
    /**
     * Loops through the engine.ImageObject from startPoint until it finds a empty pixel
